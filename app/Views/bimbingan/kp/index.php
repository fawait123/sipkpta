    ...
    <?php if ($cek != null) : ?>
        <?php if ($cek->status_perpanjang == "Perpanjang") : ?>
            <?php if ($dosbing != null && $cek->batas_bimbingan_perpanjang >= date('Y-m-d')) : ?>
                <a href="<?= base_url('bimbingan/formkp') ?>" class="btn btn-primary btn-sm mb-3">Tambah Bimbingan</a>
            <?php else : ?>
                <div class="alert alert-info alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <h5><i class="icon fas fa-info"></i> Info</h5>
                    Pastikan Anda sudah terdaftar di KP (Kerja Praktik).</br>
                    Catatan
                    <ul>
                        <li>Pastikan anda sudah Terdaftar pada matakuliah KP (Kerja Praktik).</li>
                        <li>Bimbingan hanya bisa dilakukan hanya ketika KP anda sudah di disposisi oleh KAPRODI.</li>
                        <li>Batas Bimbingan sampai tanggal <?= date('d M Y', strtotime($cek->batas_bimbingan_perpanjang)) ?></li>
                    </ul>
                </div>
            <?php endif; ?>
            ...
            <?php else : ?>
                <div class="alert alert-info alert-dismissible">
                    ...
                    <ul>
                        <li>Pastikan anda sudah Terdaftar pada matakuliah KP (Kerja Praktik).</li>
                        <li>Bimbingan hanya bisa dilakukan hanya ketika KP anda sudah di disposisi oleh KAPRODI.</li>
                        <li>Batas Bimbingan sampai tanggal <?= date('d M Y', strtotime($cek->batas_bimbingan)) ?></li>
                    </ul>
                </div>
            <?php endif; ?>
        <?php elseif ($cek->status_perpanjang == "Selesai") : ?>
            <div class="alert alert-info alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                <h5><i class="icon fas fa-info"></i> Info</h5>
                Anda Sudah Menyelesaikan Kerja Praktik.</br>
                Catatan
                <ul>
                    <li>Silahkan Mendaftar Proyek Tugas Akhir Jika sudah dibuka.</li>
                </ul>
            </div>
        <?php endif; ?>
    <?php else : ?>
                                ...
                                <select name="dosen" id="dosen" class="form-control">
                                    ...
                                    <?php foreach ($dosen as $d) : ?>
                                        <?php if (isset($_GET['dosen']) && $_GET['dosen'] == $d->nik) : ?>
                                            <option value="<?= $d->nik ?>" selected><?= $d->nik . ' - ' . $d->nama_dosen ?></option>
                                        <?php else : ?>
                                            <option value="<?= $d->nik ?>"><?= $d->nik . ' - ' . $d->nama_dosen ?></option>
                                        <?php endif ?>
                                    <?php endforeach; ?>
                                </select>
                            </div>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <?php if ($cek->no_perubahan != null && $cek->status_dosen == 'acc' && $cek->status_prodi == 'acc') : ?>
                <h6>Judul Kerja Praktik : <?= $cek->judul_perubahan ?></h6>
            <?php else : ?>
                <h6>Judul Kerja Praktik : <?= $cek->judul ?></h6>
            <?php endif; ?>
            <h6>Lokasi Kerja Praktik : <?= $cek->studi_kasus_baru != "" ? $cek->studi_kasus_baru : $cek->studi_kasus ?></h6>
            <h6>Batas Bimbingan :
                <?php if ($cek->status_perpanjang == 'Baru') : ?>
                    <span class="badge bg-danger"><?= date('d M Y', strtotime($cek->batas_bimbingan)) ?></span>
                <?php else : ?>
                    <span class="badge bg-danger"><?= date('d M Y', strtotime($cek->batas_bimbingan_perpanjang)) ?></span>
                <?php endif; ?>
            </h6>
            <a href="<?= base_url('bimbingan/download/kp') ?>" target="blank" class="btn btn-primary btn-sm">Download Bimbingan</a>
        </div>
    </div>
<?php endif; ?>
<div class="card">
                        ...
                        <tr>
                                ...
                                <td>
                                    ...
                                    <?php else : ?>
                                        ...
                                        <?php elseif ($b->status == "tolak") : ?>
                                            <span class="badge bg-danger text-white">Ditolak</span>
                                        <?php else : ?>
                                            <span class="badge bg-warning text-white">Belum direview</span>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </td>
                            <?php endif; ?>
                            <?php if (session()->get('role') == "dosen") : ?>
                                <td>
                                    <?php if ($b->metode == "online") : ?>
                                        <?php if ($b->keterangan == null) : ?>
                                            <a href="" class="btn btn-primary btn-sm review" data-toggle="modal" data-target="#modal-review" data-kd-bimbingan="<?= $b->kd_bimbingan ?>" data-npm="<?= $b->npm ?>" data-materi="<?= $b->materi ?>">Review</a>
                                        <?php else : ?>
                                    ...
                                    <?php else : ?>
                                        <form action="<?= base_url('bimbingan/accbimbingan') ?>" method="POST" id="form-acc-bimbingan">
                                            ...
                                            <input type="hidden" name="judul" value="<?= $b->materi ?>">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="judul" id="judulreview">
                    <input type="hidden" name="npm" id="npmreview">
                    <div class="form-group">
